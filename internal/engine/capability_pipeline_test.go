...
func TestDishonestRunnerCannotClaimEngineDecisionWrites(t *testing.T) {
	for _, path := range []string{"decision.html", "decisions/999.html"} {
		t.Run(path, func(t *testing.T) {
			r := &runner.FakeRunner{
				...
				OnStart: func(_, _, _, workdir string) error {
					...
					return os.WriteFile(target, []byte("agent-authored\n"), 0o644)
				},
			}
			e, s := newEngine(t, r)
			id, err := e.CreateIssue("dishonest engine output", "", "default", nil, 0, nil)
			if err != nil {
				t.Fatal(err)
			}
			stage := flow.Stage{
			...
			err = e.runStageOnce(context.Background(), e.issues[id], stage, 1, 1, nil)
			if err == nil || !strings.Contains(err.Error(), string(capability.ReasonPostStageViolation)) {
				t.Fatalf("dishonest engine output error=%v", err)
			}
			attempts, loadErr := s.StageLifecycleAttempts(id, stage.Name)
			if loadErr != nil || len(attempts) != 1 {
				t.Fatalf("attempts=%+v err=%v", attempts, loadErr)
...
type missingReapProofRunner struct {
	*runner.FakeRunner
}

func (r *missingReapProofRunner) Run(ctx context.Context, request runner.StageRequest, asks chan<- runner.Ask) <-chan runner.Result {
	source := r.FakeRunner.Run(ctx, request, asks)
	done := make(chan runner.Result, 1)
	go func() {
		result := <-source
		result.RuntimeAudit = nil
		done <- result
	}()
	return done
}

func TestStageRejectsSuccessWithoutDescendantReapProof(t *testing.T) {
	r := &missingReapProofRunner{FakeRunner: &runner.FakeRunner{
		Scripts: map[string]runner.Script{"inspect/agent": {}},
	}}
	e, s := newEngine(t, r)
	id, err := e.CreateIssue("missing reap proof", "", "default", nil, 0, nil)
	if err != nil {
		t.Fatal(err)
	}
	stage := flow.Stage{
	...
	err = e.runStageOnce(context.Background(), e.issues[id], stage, 1, 1, nil)
	if err == nil || !strings.Contains(err.Error(), string(capability.ReasonPostStageViolation)) {
		t.Fatalf("missing reap proof error=%v", err)
	}
	attempts, loadErr := s.StageLifecycleAttempts(id, stage.Name)
	if loadErr != nil || len(attempts) != 1 {
		t.Fatalf("attempts=%+v err=%v", attempts, loadErr)
	...
	if _, found, loadErr := s.StageLifecycleResult(attempts[0]); loadErr != nil || found {
		t.Fatalf("unreaped result found=%t err=%v", found, loadErr)
	}
}
...
func (r *reapingCapabilityRunner) Run(ctx context.Context, request runner.StageRequest, _ chan<- runner.Ask) <-chan runner.Result {
	...
	go func() {
		reap := []capability.AuditRecord{{ContractID: request.Contract.ContractID, Phase: "reap", Outcome: "passed"}}
		if request.Agent == "fast" {
			done <- runner.Result{RuntimeAudit: reap}
			return
		}
		<-ctx.Done()
		r.reaped.Store(true)
		done <- runner.Result{Err: ctx.Err(), RuntimeAudit: reap}
	}()
	return done
}
...
type legacyRestrictedRunner struct {
	*runner.FakeRunner
	restriction pkgs.LegacyRestrictions
}

func (r *legacyRestrictedRunner) LegacyRestrictions(string) (pkgs.LegacyRestrictions, bool) {
	return r.restriction, true
}

func TestStageCompilesLegacyAllowedToolsAsSubtractiveRestrictions(t *testing.T) {
	fixture := newPolicyRecoveryFixture(t, "none", 0)
	fixture.runner.OnStart = nil
	fixture.engine.cfg.Runner = &legacyRestrictedRunner{
		FakeRunner: fixture.runner,
		restriction: pkgs.LegacyRestrictions{
			Declared: true, DenyLocalProcess: true,
		},
	}
	if err := fixture.engine.runStageOnce(context.Background(), fixture.engine.issues[fixture.issueID], fixture.stage, 1, 1, nil); err != nil {
		t.Fatal(err)
	}
	...
	record, found, err := fixture.store.CapabilityAttempt(fixture.issueID, fixture.stage.Name, attempts[0].AttemptID)
	if err != nil || !found {
		t.Fatalf("capability record=%+v found=%t err=%v", record, found, err)
	}
	for _, operation := range record.Contract.Contract.Operations {
		if operation == capability.OpLocalProcess {
			t.Fatalf("legacy package restriction did not remove local-process authority: %+v", record.Contract.Contract.Operations)
		}
	}
}
